...
/** #if WITH_S1 songjh - distance??? ???????????? Cull?????? ?????? ?????? ?????? (??????: S1CullDistanceManager::GetCullDistance()) */
var() bool bForceNoCullDistance;
...
/** WITH_S1, songjh */
var(LOD) bool bUseForceLowLODForUser;
