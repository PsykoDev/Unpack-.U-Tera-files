...
struct native VehicleState
{
    ...
    {
        RBState=(Position=(X=0.0,Y=0.0,Z=0.0),Quaternion=(X=0.0,Y=0.0,Z=0.0,W=0.0),LinVel=(X=0.0,Y=0.0,Z=0.0),AngVel=(X=0.0,Y=0.0,Z=0.0),bNewData=0)
        ServerBrake=0
        ServerGas=0
        ServerSteering=0
        ServerRise=0
...
{
    // End:0x78
    if((bIsInverted && !bVehicleOnGround) && VSize(Velocity) <= 0.10)
    {
        // End:0x76
        if(bCanFlip)
        {
...
simulated function DisplayWheelsDebug(HUD HUD, float YL)
{
    ...
    if(I < Wheels.Length)
    {
        ...
        if((((ScreenLoc.X >= float(0)) && ScreenLoc.X < HUD.Canvas.ClipX) && ScreenLoc.Y >= float(0)) && ScreenLoc.Y < HUD.Canvas.ClipY)
        {
            ...
            HUD.Canvas.DrawText(((("SA " $ string(Wheels[I].LatSlipAngle * 57.295780)) $ " (") $ string(Wheels[I].LatSlipAngle)) $ ")");
            ...
            HUD.Canvas.DrawText(string(3.1415930 * 0.50));
            ...
            {
                ForceValue = HermiteEval(float(J) * ((3.1415930 * 0.50) / GraphScale));
            ...
            ForceValueLoc.X = ScreenLoc.X + ((Abs(Wheels[I].LatSlipAngle) / (3.1415930 * 0.50)) * GraphScale);
...
simulated function GetSVehicleDebug(out array<string> DebugInfo)
{
    ...
    DebugInfo[DebugInfo.Length] = ((("Speed: " $ string(VSize(Velocity))) $ " Unreal -- ") $ string(VSize(Velocity) * 0.04261250)) $ " MPH";
    // End:0xDA
    if(Wheels.Length > 0)
    {
...
defaultproperties
{
    ...
    HeavySuspensionShiftPercent=0.50
    MaxSpeed=2500.0
