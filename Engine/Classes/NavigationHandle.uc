...
struct native NavMeshPathParams
{
    ...
    structdefaultproperties
    {
        ...
        bAbleToSearch=false
        SearchExtent=(X=0.0,Y=0.0,Z=0.0)
        SearchLaneMultiplier=0.0
        SearchStart=(X=0.0,Y=0.0,Z=0.0)
        MaxDropHeight=0.0
        MinWalkableZ=0.0
        MaxHoverDistance=0.0
    }
};
...
{
    MaxPositions = -1;    
    ValidBoxAroundStartPos = vect(0.0, 0.0, 0.0);                                        
}
...
function DrawBreadCrumbs(optional bool bPersistent)
{
    ...
    if(bDebug_Breadcrumbs)
    {
        ...
        if(Count < 10)
        {
            curPt = Breadcrumbs[I];
            // End:0x9C
            if(curPt == vect(0.0, 0.0, 0.0))
            {
                // [Explicit Break]
                goto J0x19C;
            }
            // End:0x10D
            if((lastPt != vect(0.0, 0.0, 0.0)) && Count < (10 - 1))
            {
                Outer.DrawDebugLine(curPt, lastPt, 0, 255, 0, bPersistent);
            }
            Outer.DrawDebugBox(curPt, vect(5.0, 5.0, 5.0), 0, 0, 255, bPersistent);
            -- I;
            // End:0x17B
            if(I < 0)
            {
...
defaultproperties
{
    BreadCrumbDistanceInterval=75.0
}